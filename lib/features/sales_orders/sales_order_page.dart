...
import 'data/sales_order_line.dart';
...
bool _isValidDate(String s) => _parseStorage(s) != null;
...
/// Sales Order document page. New or existing. Summary, Overview/Lines tabs, status-based actions.
...
class _SalesOrderPageState extends State<SalesOrderPage> {
  ...
  void _load() {
    ...
    if (_order != null) {
      ...
      final stored = _parseStorage(_order!.date);
  ...
  bool get _canCreateShipment =>
      _order != null &&
      _order!.isConfirmed &&
      shipmentsRepository.canCreateShipmentForSalesOrder(_order!.id);

  String get _title => _order == null
      ? 'New Sales Order'
      : 'Sales Order ${_order!.number}';

  List<String> get _breadcrumbSegments => _order == null
      ? ['Sales', 'Sales Orders', 'New Sales Order']
      : ['Sales', 'Sales Orders', _order!.number];
  ...
  String? _validate() {
    ...
    if (_customerId == null || _customerId!.isEmpty) return 'Customer is required';
  ...
  void _save() {
    ...
    if (widget.isCreateMode || _order == null) {
      final created = _repo.add(SalesOrder(
        id: '',
        number: '',
        date: dateStr,
        customerId: _customerId!,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sales order saved')),
        );
        context.go('/${AppRoutes.pathSalesOrders}/${created.id}');
      }
      return;
    }
  ...
  void _cancel() {
    context.go('/${AppRoutes.pathSalesOrders}');
  }
  ...
  void _confirm() {
    ...
    final dateStr = _formatStorage(dateParsed);
    _repo.update(_order!.copyWith(
      date: dateStr,
      customerId: _customerId,
      warehouseId: _warehouseId,
  ...
  void _createShipment() {
    ...
    if (!shipmentsRepository.canCreateShipmentForSalesOrder(_order!.id)) return;
    final shipment = shipmentsRepository.createFromSalesOrder(_order!);
    if (mounted) {
      context.go('/${AppRoutes.pathShipments}/${shipment.id}');
    }
  }
  ...
  void _updateLine(int index, SalesOrderLine line) {
    setState(() {
        ...
        children: [
          AppPageHeader(
            ...
            breadcrumb: AppBreadcrumb(
              segments: ['Sales', 'Sales Orders', widget.id!],
            ),
            backFallbackRoute: '/${AppRoutes.pathSalesOrders}',
          ),
          const Divider(height: 1),
          const Expanded(
            child: Center(child: Text('Sales order not found')),
          ),
        ],
      );
    }
          ...
          AppPageHeader(
            ...
            actions: [
              ...
              if (widget.isCreateMode) ...[
                ...
                OutlinedButton(
                  onPressed: _canCreateShipment ? _createShipment : null,
      ...
      child: Column(
        ...
        children: [
          ...
          if (isDraft)
            ...
            _Row(
              'Date',
              order != null && _isValidDate(order!.date)
                  ? _formatDisplay(_parseStorage(order!.date)!)
                  : '—',
            ),
          const SizedBox(height: 8),
          if (isDraft) ...[
            ...
            _Row('Customer', customer?.name ?? order?.customerId ?? ''),
            const SizedBox(height: 8),
...
class _CompactCalendarOverlayState extends State<_CompactCalendarOverlay> {
  ...
  static const _popupWidth = 7 * _cellSize + 24.0;
  static const _popupHeight = 12 + 48 + 36 + 8 + 20 + 6 + 6 * _cellSize + 8 + 36 + 12;
  ...
  Widget build(BuildContext context) {
    ...
    final startWeekday = first.weekday - 1;
...
class _LinesTab extends StatefulWidget {
  ...
  final void Function(int index, SalesOrderLine line) onUpdateLine;
